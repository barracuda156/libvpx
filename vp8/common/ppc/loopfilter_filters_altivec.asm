...
; when we think of them as permutations of address bits: Concatenate
; a group of vector registers and think of it as occupying a block
; of memory beginning at address zero. The low four bits 0–3
; of the address then correspond to position within a register,
; the higher-order address bits select the register.
;
...
; 0 with 4, 1 with 5, 2 with 6, and 3 with 7. Bits 0–3 index
; the column, which is interchanged with the row addressed by bits 4–7.
;
...
; number (= high-order part of address) bits are at the disposal
; of the programmer.
;
; To avoid excess transposes, we filter all 3 vertical luma subblock
; edges together. This requires a single 16x16 transpose, which,
; in the above language, amounts to the following permutation of address
...
;
; Inverse transpose is similar, except here I -> (I+3) mod 7
; and the result lands in the "mirror" registers v10–v17
