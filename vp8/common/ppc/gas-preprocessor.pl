#!/usr/bin/perl -s
# by David Conrad
# and hacked up by Cameron Kaiser
...
sub expand_macros {
    ...
    if ($line =~ /(\S+:|)\s*([\w\d\.]+)\s*(.*)/ && exists $macro_lines{$2}) {
        ...
        # some gas files use ; as an alternate comments character.
        # this is a catch-all
        last;
...
unless ($bail) {
                    die "Too many arguments to macro $macro (got $i, should be $#{$macro_args{$macro}}) (they were: @args)";
} else {
    warn "error for macro $macro, emitting as is\n";
